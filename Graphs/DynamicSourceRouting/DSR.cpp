/*
    NickName: DPSolver ;)
    Solver: Abdelaziz Salah Mohammed 
    BN: 3, Sec: 2
*/
...
/*
    This code is trying to solve the problem using DFS, but failed :(, got only 5.2 points.
    We need to try BFS method instead.
*/
...
void addTheCorrectPath(map<int, vector<int>> &memo, const int &nei, const vector<int> &path)
{
    /// check if this neigbour was inserted in the memo before
    int oldPathSize = memo[nei].size();
    if (oldPathSize)
    {
        int currentPathSize = path.size();
        /// check if the length of the current path < previous path
        if (currentPathSize < oldPathSize)
            memo[nei] = path;
        else if (currentPathSize == oldPathSize)
        {
...
void BFS_RREQ(const vector<vector<int>> &G, const int &src, const int &dest, map<int, vector<int>> &memo, vector<int>& path)
{
    ...
    while (Q.size())
    {
        ...
        for (int nei : G[topNode])
        {
            ...
            if (nei == dest)
                continue;
            /// add the current nei to the current path
            path.push_back(nei);

            addTheCorrectPath(memo, nei, path); 
...
int main()
{
    ...
    /// 4. create an empty vector which contains the path
    vector<int> path;
